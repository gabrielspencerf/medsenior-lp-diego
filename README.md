...
- **`VITE_SITE_URL`** — URL pública do site (`https://…`, sem barra final). Usada em SEO (`SeoHead`), canonical/Open Graph no runtime e, **no build**, injeção de `og:image` / `twitter:image` / `canonical` no `index.html` (ver `vite.config.ts`).
